...
public:
    VerticalMover(sf::RenderWindow& window, sf::Vector2f position, sf::Vector2f size)
    : mRenderWindow(window), mInitialY(position.y)
    {
        mShape.setSize(size);
    ...
    void update(float value) override
    {
        float newY = mInitialY - (value * 5.0f);
        mShape.setPosition(mShape.getPosition().x, newY);
    }
};
...
int main()
{
    ...
    std::vector<Observer*> observers
    {
        ...
        new VerticalMover{window, {400, 400}, {80, 30}}
    };
